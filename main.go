...
func main() {
	...
	if err != nil {
		log.Fatal(
			"Gagal membuat koneksi database:",
			err,
		)
	}
	...
	log.Println(
		"Railway  : iriguchi.proxy.rlwy.net:53927",
	)
	...
	go func() {
		...
		log.Println(
			"gRPC server berjalan di 0.0.0.0:50051...",
		)
	...
	go func() {
		...
		log.Println(
			"HTTP server berjalan di 0.0.0.0:" +
				port +
				"...",
		)
...
func penyanyiHandler(
	w http.ResponseWriter,
	r *http.Request,
) {

	id, hasID := getIDFromPath(
		r.URL.Path,
	)
